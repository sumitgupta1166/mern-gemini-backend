...
export const textSearch = asyncHandler(async (req, res) => {
  ...
  const docs = await Doc.find({ $text: { $search: q } })
    .limit(50)
...
export const semanticSearch = asyncHandler(async (req, res) => {
  ...
  if (!qEmb)
    return res.status(200).json(new ApiResponse(200, [], "Embedding failed"));
  // load docs with embeddings
  const docs = await Doc.find({ embedding: { $exists: true, $ne: [] } })
    .limit(500)
    .populate("createdBy", "name");
  const scored = docs
    .map((d) => ({ doc: d, score: cosine(qEmb, d.embedding || []) }))
    .filter((x) => x.score > -1);
  scored.sort((a, b) => b.score - a.score);
  const top = scored
    .slice(0, parseInt(req.query.limit || "10", 10))
    .map((s) => ({ ...s.doc.toObject(), score: s.score }));
