...
if ((!tags || tags.length === 0) && summary) tags = await ai.generateTags(`$
{title}\n\n${summary}`).catch(() => []);
// try to compute embedding
const emb = await ai.embed(`${title}\n\n${summary || content}`).catch(() =>
null);
if (Array.isArray(emb)) embedding = emb;
} catch (e) {
console.warn("AI enrichment failed:", e.message);
}
const doc = await Doc.create({ title, content, summary, tags, createdBy,
embedding });
...
const docs = await Doc.find({}).sort({ updatedAt:
-1 }).skip(skip).limit(limit).populate("createdBy", "name email");
return res.status(200).json(new ApiResponse(200, { data: docs, page, limit },
"Docs"));
});
...
if (doc.createdBy.toString() !== req.user._id.toString() && req.user.role !==
"admin") {
...
doc.versions.unshift({ title: doc.title, content: doc.content, summary:
doc.summary, tags: doc.tags, createdBy: doc.createdBy });
...
const tags = (await ai.generateTags(`${doc.title}\n\n${summary ||
doc.content}`).catch(() => [])) || [];
const emb = await ai.embed(`${doc.title}\n\n${summary || doc.content}
`).catch(() => null);
...
if (!doc) throw new ApiError(404, "Doc not found");
return res.status(200).json(new ApiResponse(200, doc.versions || [],
"Versions"));
});